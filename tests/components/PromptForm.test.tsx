...
describe("PromptForm", () => {
  it("validates required prompt before submit", async () => {
    ...
    expect(await screen.findByRole("alert")).toHaveTextContent("请输入 prompt。");
  });
  ...
  it("submits prompt-only requests", async () => {
    ...
    await userEvent.type(screen.getByLabelText("Prompt"), "a tiny robot");
    await userEvent.click(screen.getByRole("button", { name: "开始生成" }));
  ...
  it("submits multiple images for reference requests", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch");
    fetchMock
      .mockResolvedValueOnce(new Response(JSON.stringify(publicConfig), { status: 200 }))
      .mockResolvedValueOnce(
    ...
    await userEvent.upload(screen.getByLabelText("上传图片"), [
      ...
      new File(["image-2"], "image-2.webp", { type: "image/webp" }),
    ]);
    await userEvent.type(screen.getByLabelText("Prompt"), "reference prompt");
    await userEvent.click(screen.getByRole("button", { name: "开始生成" }));
  ...
  it("clears selected images when switching back to generate mode", async () => {
    ...
    expect(screen.queryByLabelText("上传图片")).not.toBeInTheDocument();
    await userEvent.type(screen.getByLabelText("Prompt"), "generate prompt");
    await userEvent.click(screen.getByRole("button", { name: "开始生成" }));
  ...
  it("hides API settings and omits them from sealed submissions", async () => {
    ...
    await userEvent.type(screen.getByLabelText("Prompt"), "sealed prompt");
    await userEvent.click(screen.getByRole("button", { name: "开始生成" }));
